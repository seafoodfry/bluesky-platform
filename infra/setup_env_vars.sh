...
if [ "$GPG_OK" = "1" ]; then
    ...
    export TF_VAR_github_username=$(op read "op://eng-vault/bluesky-platform-deploy-key/username")
    export TF_VAR_github_deploy_private_key=$(op read "op://eng-vault/bluesky-platform-deploy-key/private key")
